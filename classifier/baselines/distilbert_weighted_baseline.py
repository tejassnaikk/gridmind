"""
DistilBERT fine-tune baseline (v2 — class-weighted loss) for the GridMind
obligation classifier.

Identical to distilbert_baseline.py in every respect except the loss
function: a class-weighted CrossEntropyLoss with weights [1.0, n_neg/n_pos]
computed per training fold replaces the default unweighted cross-entropy.
All hyperparameters, seeds, fold assignments, tokenizer calls, optimizer,
and TrainingArguments are unchanged.

Usage:
    python -m classifier.baselines.distilbert_weighted_baseline
"""
...
class WeightedTrainer(Trainer):
    """Trainer subclass that applies a per-fold class-weighted CrossEntropyLoss."""
...
def main() -> None:
    ...
    for fold_idx, (train_idx, val_idx) in enumerate(cv.split(bodies, labels)):
        ...
        fold_n_pos = sum(y_train)
        fold_n_neg = len(y_train) - fold_n_pos
        class_weights = torch.tensor(
            [1.0, fold_n_neg / fold_n_pos], dtype=torch.float32
        )
        print(
            f"  fold {fold_idx+1} class_weights: "
            f"[{class_weights[0]:.3f}, {class_weights[1]:.3f}]  "
            f"({fold_n_neg} neg / {fold_n_pos} pos in train)"
        )
    ...
    # ------------------------------------------------------------------ #
    print("=" * 62)
    print("  GridMind DistilBERT v2 (class-weighted) — Obligation Classifier")
    print("=" * 62)
    ...
    _show_group("DistilBERT (class-weighted)=1, human=0  [false positives]", false_positives)
    _show_group("DistilBERT (class-weighted)=0, human=1  [false negatives]", false_negatives)
    ...
    delta_v1      = f1 - _DISTILBERT_V1_F1

    print("\n── Comparison: all four baselines ────────────────────")
    print(f"  {'Model':<44}  {'F1':>6}")
    print(f"  {'-'*44}  {'------'}")
    print(f"  {'Regex (shall → 1)':<44}  {_REGEX_F1:>6.3f}")
    print(f"  {'TF-IDF + LogReg (OOF CV)':<44}  {_LOGREG_F1:>6.3f}")
    print(f"  {'DistilBERT v1 unweighted (OOF CV)':<44}  {_DISTILBERT_V1_F1:>6.3f}")
    print(f"  {'DistilBERT v2 class-weighted (OOF CV)':<44}  {f1:>6.3f}")
    print(f"  {'Delta vs regex baseline':<44}  {'▲' if delta_regex  > 0 else '▼'} {abs(delta_regex):.3f}")
    print(f"  {'Delta vs LogReg baseline':<44}  {'▲' if delta_logreg > 0 else '▼'} {abs(delta_logreg):.3f}")
    print(f"  {'Delta vs DistilBERT v1':<44}  {'▲' if delta_v1     > 0 else '▼'} {abs(delta_v1):.3f}")
    print()
